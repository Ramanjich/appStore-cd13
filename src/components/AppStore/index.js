...
class AppStore extends Component {
  state = {selecttab: tabsList[0].tabId, searchInput: ''}
  ...
  onTabchange = tabId => {
    this.setState({selecttab: tabId})
  }

  render() {
    const {selecttab, searchInput} = this.state
    const updatedList = appsList.filter(eachOne =>
      eachOne.appName.toLowerCase().includes(searchInput),
    )
    const selectList = updatedList.filter(
      eachapp => eachapp.category === selecttab,
    )
    return (
          ...
          <ul className="tab-container">
            {tabsList.map(eachtab => (
              <Tabitem
                ...
                isActive={selecttab === eachtab.tabId}
              />
            ))}
          </ul>
          <div className="appitems-container">
            <ul className="miniapp-container">
              {selectList.map(eachitem => (
